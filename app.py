...
.stApp {{
    background-image: linear-gradient(rgba(2,6,23,0.82), rgba(2,6,23,0.82)),
...
.main-title{{
    ...
    font-weight:900;
    color:white;
    text-shadow:0 0 18px #00e5ff, 0 0 35px #ff00ff;
}}
...
if check:
    if message.strip():
        ...
        st.session_state.reasons = list(dict.fromkeys(matched_words))[:5]
    ...
    def result_popup():
        ...
        if st.session_state.result == "SPAM":
            ...
            if st.session_state.reasons:
                st.write("### 🔍 Why detected as spam?")

                st.write(
                    "These words matched patterns commonly found in spam messages from the training dataset:"
                )

                for word in st.session_state.reasons:
                    st.write(f"• {word}")
...
⚠️ <b>Disclaimer:</b> This Email & SMS Spam Detector provides predictions using machine learning and has approximately
<b style='color:#00e5ff;'> This Email & SMS Spam Detector predicts messages using a machine learning model trained on a spam message dataset.</b>.
